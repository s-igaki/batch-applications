...
### 2. ニュースクローラー (`news_crawler.py`)
- **対象**: AI、フィンテック、テクノロジー、ビジネス・経済、政治・国際情勢
- **実行**: 手動実行
- **保存先**: `news_data/`

### 3. コンテンツ情報クローラー (`content_crawler.py`)
- **対象**:
  - 映画（新作、評判）
  - レストラン・カフェ（新規オープン、人気店）
  - 商業施設（オープン情報、リニューアル）
- **実行**: 毎日 08:00 JST
- **保存先**: `content_data/`

### 4. エンジニア情報クローラー (`engineer_crawler.py`)
- **対象**:
  - 技術トレンド（プログラミング言語、フレームワーク、GitHub人気）
  - 開発ツール・サービス（新リリース、アップデート）
  - 技術カンファレンス・イベント
  - AI・機械学習（新モデル、論文）
  - クラウド・インフラ（AWS、Azure、GCP）
  - セキュリティ（脆弱性、パッチ情報）
- **実行**: 毎日 09:00 JST
- **保存先**: `engineer_data/`
...
# エンジニア情報を収集
python engineer_crawler.py

# 強制再実行（既に今日のデータがある場合）
python content_crawler.py --force
```

### GitHub Actionsで自動実行

- **不動産**: 毎日 06:00 JST 自動実行
- **コンテンツ**: 毎日 08:00 JST 自動実行
- **エンジニア**: 毎日 09:00 JST 自動実行
